...
- JD 每张图仅监督其 23 个已标注维度；字典集为安全互斥组提供正负监督；只有正样本且缺少可信负例的字典词进入 PU 目标；其余维度保持 unknown。
- `known_mask=1` 的位置计算 masked BCE；`pu_positive_mask=1` 的位置使用正例–未标注 pairwise ranking 风险，未标注池只提供相对排序参照。任何 unknown 均不得直接计为 0。
- 完全相同或视觉相同的图片先聚合全部正标签，再进入切分。exact SHA 和 exact pHash 用于监督合并；pHash Hamming 距离 ≤2 的连通分量用于 split 隔离。
- 跨来源同标签冲突遵循可信正例优先；同一安全互斥组出现两个及以上不同正标签时，该组相关维度全部改为 unknown；所有冲突均记录审计表。
- 安全互斥组仅包含：领型 `立领/翻领/无领`、肩部结构 `插肩袖/正肩袖/落肩袖`、外廓形 `H型/O型/X型/A型/茧型/箱型`、松量 `合体/宽松`、衣长 `长款/中长款/中款/短款`。`弯袖`不加入肩部互斥组；压胶工艺不互斥。
- 允许两类严格蕴含负例：已知 `连帽=0` 可推出 `拆卸帽=0`、`帽口抽绳=0`；`无袖=1` 可推出所有袖型/袖口标签为 0，任一袖型/袖口已知正例可推出 `无袖=0`。不得反向把 `连帽=1` 推成拆卸帽或帽口抽绳正例。
...
7. 每行 labels、known_mask、pu_positive_mask 均严格为 57 维，schema 哈希一致；非互斥字典正例仅进入 PU 正例掩码。
...
3. pos_weight 仅按训练集已知正负计数计算并设置上限；20 个缺少可信 class prior 的 PU 标签使用 FP32 pairwise positive-vs-unlabeled ranking 风险与均匀未标注采样，避免正例-only目标塌缩为全预测 1。PU 分数明确标为未校准置信度。
